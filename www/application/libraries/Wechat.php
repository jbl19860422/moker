...
class Wechat
{
	...
	const SHAKEAROUND_DEVICE_BINDLOCATION = '/shakearound/device/bindlocation?';//配置设备与店铺ID的关系
	...
	public function getRev()
	{
		...
		//setcache('receive', $this->_receive, 'notify');
		return $this;
	}
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    /**
     * 批量导入店铺信息
	 * @tutorial 返回插入的店铺id列表，以逗号分隔。如果有插入失败的，则为-1，请自行核查是哪个插入失败
	 * @param array $data    数组形式的json数据，由于内容较多，具体内容格式请查看 微信卡券接口文档
	 * @return boolean|string 成功返回插入的店铺id列表
     */
    ...
    /**
     * [searchShakeAroundDevice 查询已有的设备ID、UUID、Major、Minor、激活状态、备注信息、关联店铺、关联页面等信息。
     ...
     * poi_id 店铺ID，有的话则返回，没有的话不会在JSON 格式内
     * @access public
     * @author polo<gao.bo168@gmail.com>
