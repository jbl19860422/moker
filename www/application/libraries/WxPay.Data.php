...
class WxPayDataBase
{
	...
	**/
	...
	public function MakeSign()
	{
		...
		//签名步骤二：在string后加入KEY
		$string = $string . "&key=".WxPayConfig::MKEY();
		//file_put_contents(dirname(dirname(__FILE__))."/logs/maked-sign-string.".time().".txt", $string);
		//签名步骤三：MD5加密
		$string = md5($string);
		//签名步骤四：所有字符转为大写
...
class WxPayResults extends WxPayDataBase
{
	...
	public function CheckSign()
	{
		...
		//file_put_contents(dirname(dirname(__FILE__))."/logs/error_sign.".time().".txt", $this->GetSign() . '?=' .$sign);
		throw new WxPayException("签名错误,fuck");
	}
