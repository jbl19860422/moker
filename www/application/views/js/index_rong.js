...
var GUI = {
	...
	*/
	updateSinger:function(singer) {
		ReactUpdate.header('singer', singer);
	},
	...
	*/
	updateOnlineUsers:function(onlineUsers) {
		ReactUpdate.header('users', onlineUsers);
	},

	updateBarView:function() {
		$(".diy-user img").attr("src", BAR.barinfo.bar_img);
		$(".diy-user .name").html(BAR.barinfo.bar_name);
	},
	...
	*/
	showUserInfo:function(user) {
		if(user) {
			...
			API.query_user_info(GUEST.userinfo.user_id, userId, function(data) {
				...
				ReactUpdate.userModal('user', data);
	...
	addNormalMsg:function(message, bAtMsg) {
		...
		console.log(message);
		$(".diy-msg").append('<li class="margin-bottom hd-grid animated user-message bounceInLeft" msg-id="'+message.message_id+'" user-id="'+
			message.user_id+'" onclick="GUI.showUserInfo('+message.user_id+')"><div class="message" style="background-image:url('+message.headimg+')"><div><span class="label label-level-1">32</span><span class="text-gray">'+
			message.nickname+'</span><br/><span class="text-black hd-h5">'+
			msg+'</span></div></div></li>');
		scrollBottom();
	},
	//添加弹幕消息
	addBarrageMsg:function(message, bAtMsg) {
	    ...
	    var html = '<div class="barrage-item barrage-in-out '+barrageNumClass+'">' +
	                    ...
	                    '<p class="danmu-user"><span class="text-white text-ellipsis">'+message.nickname+'</span><span class="label label-level-2">18级</span></p>' +
	                    '<p class="text-white danmu-text text-ellipsis">'+msg+'</p>'+
	               '</div>';
	...
	addLoginMsg:function(user) {
		console.log(user);
		$(".diy-msg").append('<li class="margin-bottom text-white animated bounceInLeft"><span class="label label-yellow">32级</span>'+ user.nickname +'进入了直播间</li>');
		scrollBottom();
	},
	//添加礼物消息
	addGiftMessage: function(obj) {
		var addEl = $('<li class="diy-sys-3 margin-bottom user-message" onclick="GUI.showUserInfo('+obj.user_id+')"> \
			<img class="radius-100 width-40 height-40 fl-l" src="'+obj.headimg+'" /> \
			<p class="text-ellipsis fl-l text-white padding-right margin-small-left"><span class="label label-level-3">7级</span>'+obj.nickname+'</p> \
			<p class="padding-leftt text-white text-indent-5">送出'+obj.item_name+'</p>\
	...
	addLoveMsg:function(obj) {
		console.log(obj);
		$(".diy-msg").append('<li class="margin-bottom user-message animated bounceInLeft" user-id="'+obj.user_id+'"><span class="label label-level-1">32</span><span class="text-white">'+obj.nickname+'</span> <span class="text-yellow">'+"给"+obj.target_nickname+"加了"+obj.count+"个爱心"+'</span></li>');
	...
	showGiftPanel : function() {
		...
		$(".diy-cryptolalia").addClass("hide");
		$(".diy-gift ul.gifts").html("");
		$(".diy-gift .gift-send").removeClass("gift-selected");
		console.log(items);
		for(var i = 0; i < items.length; i++) {
			if(GUEST.recvUser.role.indexOf(items[i].type) >= 0) {
				if(items[i].price > 0) {
					$(".diy-gift ul.gifts").append('<li data-item="'+items[i].item_id+'" '+(items[i].gif?('data-gif="'+items[i].gif+'"'):'')+'> \
											<div class="img"> \
...
function CONTROLLER() {
	...
	this.sendMsg = function() {
		var chatContent = $(".diy-chat .chat").val();
	...
	//发送聊天信息
	$("body").on("click", ".diy-chat .sendmsg-btn", _this.sendMsg);
	//弹幕开关
	$("body").on("click", ".barrage-btn", GUI.switchBarrage);
	//增加歌手爱心值
	...
	//所有人
	$("#ID_onlineCount").click(function() {
		window.location.href = "http://dream.waimaipu.cn/index.php/user/alluser";
	});
}
