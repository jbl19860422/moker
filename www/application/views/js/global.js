...
// $("body").click(function() {
// 	if(g_curPage == ".page-recharge") {
// 		alert(event.srcElement.tagName);
// 		if(event.srcElement.tagName == "BODY") {//显示二维码
// 			FN.loading_clickhide.show();
// 		}
// 	}
// });
...
function query_activity() {
	$.post(
			...
			function(json) {
				...
				// alert(time_start+" "+cur_time+" "+time_end);
...
function initWebSocket() {
	...
	g_socket.on('reconnect', function() {
		//发送登录消息给服务器
		g_socket.emit('login', {
			bar_id:g_bar.bar_id,
			user_id:g_guest.user_id,
	...
	g_socket.on('barMessage', function(obj) {
		$(".diy-sys").append('<li class="diy-sys-2 padding-left padding-right margin-bottom msg-in-out"> \
								<a href="javascript:void(0)" class="text-red" onclick="switchToBarMsgPage()" style="text-decoration: none;">商家公告：'+obj.title+' \
							  </a></li>');
	...
	g_socket.on('sysMessage', function(obj) {
		$(".diy-sys").append('<li class="diy-sys-2 padding-left padding-right margin-bottom msg-in-out"> \
								<a href="javascript:void(0);" onclick="switchToSysMsgPage()" class="text-red" style="text-decoration: none;">系统消息：'+obj.title+' \
							  </a></li>');
		g_sysMessage.push(obj);
	});
