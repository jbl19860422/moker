...
var GUEST = {
	...
	* 初始化
	*/
	init:function(user_id, desk_id, nickname, headimg, role, sex, bShowBarrageAlert) {
		...
		GUEST.websocket = io.connect('ws://imoke.live:3000');

		//WEBSOCKET 消息
		GUEST.websocket.on('connect', function() {
			GUEST.websocket.emit('login', {
		...
		* 重新连接上后
		*/
		GUEST.websocket.on('reconnect', function() {
			//发送登录消息给服务器
			GUEST.websocket.emit('login', {
				bar_id:BAR.bar_id,
		...
		* 新用户登录
		*/
		GUEST.websocket.on('login', function(obj) {
			BAR.setOnlineUsers(obj.guest);//更新在线人数
		...
		*	消息
		*/
		GUEST.websocket.on('message', function(obj) {
			if(obj.type == 'normsg') {
		...
		* 歌手切换
		*/
		GUEST.websocket.on('singerSwitch', function(obj) {
			obj.role = 'a';
		...
		*  爱心值增加
		*/
		GUEST.websocket.on('addlove_msg', function(obj) {
			GUI.addLoveMsg(obj);
		});

		//爱心值修改消息
		GUEST.websocket.on('addlove', function(obj) {
			obj.role = 'a';
			GUI.updateSinger(obj);
		});
		//礼物消息
		GUEST.websocket.on('giftMessage', function(obj) {
			GUI.addGiftMessage(obj);
		});
		//密语消息
		GUEST.websocket.on('priv_msg', function(message) {
			//放到对应的消息数组中
	...
	delFromPrivMsg : function(from_user_id) {
		...
		for(var i = 0; i < GUEST.privMsgs.length; i++) {
			if(GUEST.privMsgs[i].from_user_id == GUEST.userinfo.user_id) {
				for(var j = 0; j < GUEST.privMsgs[i].msgs.length; j++) {
					if(GUEST.privMsgs[i].msgs[j].to_user_id == from_user_id) {//删除我发给对方的消息
						GUEST.privMsgs[i].msgs.splice(j,1);
					}
				}
			}
		}
	...
	action:{
		...
		sendMessage:function(isBarrage, content) {
			...
			if(isBarrage) {
				GUEST.action.consumeCoin(1, function(msg) {
										GUEST.websocket.emit(msgType, msg);
									}, 
									msg
								);
			} else {
		...
		*/
		sendLoveAddMsg: function(recvUser, count) {
			GUEST.websocket.emit('addlove_msg', {
				bar_id:BAR.barinfo.bar_id,
...
var GUI = {
	...
	*/
	showPrivChatPanel:function(recvUser) {
		...
		if(recvUser) {
			...
			$(".diy-cryp-data").html("");
			GUEST.delFromPrivMsg(recvUser.user_id);//删除关于接收者的旧消息
			//查询相互消息
			//alert(recvUser.user_id);
			API.query_user_privmsg(GUEST.userinfo.user_id, recvUser.user_id, function(userinfo, records) {
				if(records) {
	...
	*/
	updateSinger:function(singer) {
		...
		$(".diy-user .uname").html(singer.nickname);
		$(".diy-user .collect").html(singer.liveness);
		$(".diy-user .zan").html(singer.lovecount);
	...
	*/
	showUserInfo:function(user) {
		if(user) {
			commonJS.cover();
			GUEST.setRecvUser(user);
			API.query_user_info(GUEST.userinfo.user_id, user.user_id, function(data) {
			...
			if(item.repeatable) {//可连发
				GiftRepeator.startDetectTick(item.item_id,function() {//第一次点击时触发调用
					...
					API.sendGift(BAR.barinfo, GUEST.userinfo, GUEST.recvUser, {item_id:item_id, item_count:GiftRepeator.clickedCount}, function() {
						GUEST.websocket.emit('giftMessage', {
									type:'giftMessage',
...
$(function() {
	...
	API.query_unviewed_privmsg(GUEST.userinfo.user_id, function(data) {
		...
		$("#ID_msg_count").html(GUEST.unviewdPrivMsgCount);
		//是否显示和某个用户的密语
		if($.cookie('privmsg_user') && $.cookie('privmsg_user')!=null && $.cookie('privmsg_user')!=undefined && $.cookie('privmsg_user')!="" && $.cookie('privmsg_user')!="null") {
			API.query_user_info($.cookie("user_id"), $.cookie('privmsg_user'), function(userinfo) {
				userinfo.user_id = $.cookie('privmsg_user');
				GUEST.setRecvUser(userinfo);
				GUI.showPrivChatPanel(GUEST.recvUser);
			});
			// API.query_user_privmsg(GUEST.userinfo.user_id, $.cookie('privmsg_user'), function(userinfo, records) {
			// 	userinfo.nickname = userinfo.nick;//昵称，先这么处理
			// 	GUEST.setRecvUser(userinfo);
			// 	if(records) {
			// 		for(var i = 0; i < records.length; i++) {
			// 			GUEST.addToPrivMsg(records[i]);
