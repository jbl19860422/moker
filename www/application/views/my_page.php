...
<div class="page-center bg-gray hd-h3">
	...
	<div class="diy-userinfo text-c bg-white margin-bottom-15">
		<div class="headimg margin-bottom-15"><img class="radius-100" src=""/></div>
...
<script type="text/javascript">
	var g_userinfo = <?php echo json_encode($user_info);?>;
	var g_barmessage = <?php echo json_encode($bar_message);?>;
	var g_sysmessage = <?php echo json_encode($sys_message);?>;
	var g_bar = <?php echo json_encode($bar_info);?>;
	var g_chatRecords = <?php echo json_encode($chatRecords);?>;
</script>
<script type="text/javascript">
$(function(){
...
<script type="text/javascript">
$(function() {
	$(".nickname div").html(g_userinfo.nickname);
	$(".headimg img").attr('src', g_userinfo.headimg);
	$(".page-center .diy-usermsg .money-num").html(g_userinfo.money);
	...
	$(".diy-userinfo .zan").html(g_userinfo.love_count);
	$(".diy-userinfo .collect").html(g_userinfo.liveness);

	$(".goback").click(function(){history.back(-1);});

	var last_msg = {};
	for(var p in g_barmessage) {
		last_msg = g_barmessage[p];
	}
	$(".newest-bar-msg .introl").html(JSON.parse(last_msg).title);

	var msgs = [];
	for(var p in g_sysmessage) {
		msgs.push({
			time:p,
			data:g_sysmessage[p]
		});
	...
	//酒吧活动
	var time_start = parseInt(g_bar.activity_start);
	var time_end = parseInt(g_bar.activity_end);
	var cur_time = parseInt(g_bar.cur_time);
	if(time_start < cur_time && cur_time < time_end) {
		$("#ID_activity div").html("HI现场("+g_bar.activity_name+"正在进行)");
		$("#ID_activity").attr('href', g_bar.activity_phoneurl);
	} else {
	...
	$(".priv-msg .num").html(g_chatRecords.length);
	if(g_chatRecords.length > 0) {
		g_chatRecords.sort(function(a,b) {
			return b.timestamp - a.timestamp;
		});
		$(".priv-msg .introl").html(g_chatRecords[0].content);
	}
